...
export async function extractTextFields(
    ...
    if (existingTag) {
      ...
      try {
        fileForm
          ...
          .setText(existingTag?.output ?? "BSP_TEXT");
      } catch (error) {
  ...
  fileForm.acroForm.dict.set(PDFName.of("NeedAppearances"), PDFBool.True);
    ...
    switch (bookPart) {
      ...
      case "PLANNER":
        if (pageCount < 2 || pageCount > 92) {
          return {
            valid: false,
            message: "Der Planer muss zwischen 2 und 92 Seiten haben",
          };
        }
        break;
      case "BINDING":
        if (pageCount >= 1) {
          return {
            valid: false,
            message: "Der Einband darf keine Seiten enthalten.",
          };
        }
        break;
      default:
        if (pageCount < 1 || pageCount > 100) {
          return {
            valid: false,
            message:
              "Das Dokument muss mindestens 1 und höchstens 100 Seiten haben.",
          };
        }
    }
...
export const urlToFile = async (url: string, fileName: string) => {
  try {
    ...
    if (!response.ok)
      throw new Error(
        `Failed to fetch the file: ${response.status} ${response.statusText} ${JSON.stringify(response)}`,
      );
