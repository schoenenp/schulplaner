...
import { createCustomCoverPdf } from "@/server/util/pdf/custom-cover";
import { validatePDFUpload } from "@/server/util/pdf/functions";
import { uploadData, type FileItem } from "@/server/util/upload/functions";
import {
...
async function toUploadItem(file: File, role: UploadRole): Promise<UploadItem> {
  ...
  return {
    role,
    data: buffer,
    name: `${role === "file" ? "file" : "thumb"}_${rand}`,
  };
}
  ...
  return [
    {
      ...
      name: `${CUSTOM_COVER_FILE_PREFIX}${rand}.pdf`,
    },
    {
      role: "thumbnail",
      data: imageBuffer,
      name: `${CUSTOM_COVER_THUMB_PREFIX}${rand}.${getUploadExtension(file)}`,
    },
  ];
}
